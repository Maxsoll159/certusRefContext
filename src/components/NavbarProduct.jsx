...
export const NavbarProduct = () => {
    ...
    const { cartItems } = useContext(UsuarioContext)

    const total = cartItems.reduce((accumulator, valor) => accumulator + valor.price * valor.cantidad , 0)

    console.log("total", cartItems)

    return (
        <div className="flex flex-wrap -mx-3 mb-5">
            <div className="px-3 mb-6  mx-auto w-full bg-white rounded-xl">
                <div className="sm:flex items-stretch justify-between grow lg:mb-0  py-5 px-5">
                    ...
                    <div className="flex items-center lg:shrink-0 lg:flex-nowrap">
                        ...
                        <div className="relative flex items-center ml-2 lg:ml-4">
                            <a  className="flex items-center justify-center w-12 h-12 text-base font-medium leading-normal text-center align-middle transition-colors text-white bg-red-500 duration-150 ease-in-out bg-transparent border border-solid shadow-none cursor-pointer rounded-2xl  border-stone-200 hover:text-primary active:text-primary focus:text-primary">
                                {
                                    Math.floor(total)
                                }
                            </a>
